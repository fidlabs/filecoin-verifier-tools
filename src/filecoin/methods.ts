...
async function make(testnet) {
  ...
  */
  ...
  async function signTx(client, indexAccount, walletContext, tx) {
    const head = await client.chainHead()
    const address = (await walletContext.getAccounts())[
      indexAccount
    ]
  ...
  function decode(schema, data) {
    ...
    if (schema === 'bigint-signed') {
      return hamt.bytesToBig(data) / BigInt(2)
    }
    if (schema === 'bigint-key') {
      return hamt.readVarInt(data, 0) / BigInt(2)
    }
    if (
      schema === 'int' ||
            schema === 'buffer' ||
    ...
    if (
      schema === 'cid' &&
            data instanceof cbor.Tagged &&
            data.tag === 42
    ) {
      return cid.CID.decode(data.value.slice(1))
    }
    if (schema === 'cid') {
      return data['/']
    }
    ...
    if (schema.type === 'hamt') {
      return {
        find: async (lookup, key) => {
          const res = await hamt.find(
            data,
            lookup,
            encode(schema.key, key),
          )
        ...
        asObject: async (lookup) => {
          const res = {}
          await hamt.forEach(data, lookup, async (k, v) => {
            res[decode(schema.key, k)] = decode(
              schema.value,
              v,
            )
          })
    ...
    if (schema instanceof Array) {
      ...
      if (schema[0] === 'ref') {
        return async (load) =>
          decode(schema[1], await load(data['/']))
      }
      if (schema[0] === 'cbor') {
      ...
      if (schema.length !== data.length) {
        throw new Error(
          'schema and data length do not match',
        )
      }
      if (isType(schema[0])) {
        const res = []
  ...
  function encode(schema, data) {
    ...
    if (schema === 'cid') {
      return new cbor.Tagged(
        42,
        Buffer.concat([Buffer.from([0]), data.bytes]),
      )
    }
    if (schema === 'bool') {
      return data
...
export const methods = async () => {
  return {
    mainnet: await make(false),
    testnet: await make(true),
  }
}
